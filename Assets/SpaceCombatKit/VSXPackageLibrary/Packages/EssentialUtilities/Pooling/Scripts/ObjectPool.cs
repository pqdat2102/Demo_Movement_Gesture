...
namespace VSX.Pooling 
{
	...
	public class ObjectPool : MonoBehaviour 
	{
		...
		protected Transform inactiveObjectsParent;
        protected Transform activeObjectsParent;


        protected virtual void Awake()
		{
            inactiveObjectsParent = new GameObject("Inactive Objects Parent").transform;
            inactiveObjectsParent.SetParent(transform);
            inactiveObjectsParent.localPosition = Vector3.zero;
            inactiveObjectsParent.localRotation = Quaternion.identity;
            inactiveObjectsParent.gameObject.SetActive(false);

            activeObjectsParent = new GameObject("Active Objects Parent").transform;
            activeObjectsParent.SetParent(transform);
            activeObjectsParent.localPosition = Vector3.zero;
            activeObjectsParent.localRotation = Quaternion.identity;
        }
		...
		public virtual GameObject Get (Vector3 pos = default(Vector3), Quaternion rot = default(Quaternion), Transform parent = null, bool activate = true)
		{
			...
			for (int i = 0; i < objectList.Count; ++i)
			{
				if (!objectList[i].CachedGameObject.activeSelf)
				{
					...
					objectList[i].CachedGameObject.SetActive(activate);
					...
					{
                        objectList[i].CachedTransform.SetParent(activeObjectsParent);
                    }
			...
			{
                newPooledObject.CachedTransform.SetParent(activeObjectsParent);
            }
		...
		protected virtual PooledObject CreateNewItem(bool activate = false)
		{
			...
			GameObject newObject = Instantiate (prefab, Vector3.zero, Quaternion.identity, inactiveObjectsParent);
