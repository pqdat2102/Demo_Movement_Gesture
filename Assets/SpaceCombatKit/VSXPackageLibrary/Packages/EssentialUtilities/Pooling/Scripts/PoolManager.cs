...
namespace VSX.Pooling 
{
	...
	public class PoolManager : MonoBehaviour 
	{
		...
		protected virtual void Awake()
		{
			...
			ObjectPool[] objectPoolsInScene = FindObjectsByType<ObjectPool>(FindObjectsSortMode.None);
        ...
        public virtual GameObject Get(GameObject prefab, Vector3 pos = default(Vector3), Quaternion rot = default(Quaternion), Transform parent = null, bool activate = true)
		{
			...
			for (int i = 0; i < objectPools.Count; ++i)
			{
				...
				{
					return (objectPools[i].Get(pos, rot, parent, activate));
				}
			}
				
			// Get from new pool
			ObjectPool newPool = GetObjectPool(prefab);
			return (newPool.Get(pos, rot, parent, activate));

		}


		public virtual GameObject Get(GameObject prefab, Transform parent, bool activate = true)
		{
			...
			for (int i = 0; i < objectPools.Count; ++i)
			{
				if (objectPools[i].Prefab.name == prefab.name)
				{
					return (objectPools[i].Get(parent.position, parent.rotation, parent, activate));
				}
			}

			// Get from new pool
			ObjectPool newPool = GetObjectPool(prefab);
			return (newPool.Get(parent.position, parent.rotation, parent, activate));
		...
		/// <param name="parent">The parent for the returned item.</param>
		/// <returns>The GameObject reference of the returned item.</returns>
		public virtual GameObject Get(string prefabResourceName, Vector3 pos = default(Vector3), Quaternion rot = default(Quaternion), Transform parent = null, bool activate = true)
		{
