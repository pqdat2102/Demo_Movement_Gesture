...
namespace VSX.Utilities
{
    ...
    public class GimbalController : MonoBehaviour
    {
        ...
        {
            // Apply constraints
            float horizontalPivotAngle = WrapTo180Split(horizontalPivotLocalRotation.eulerAngles.y);
            float verticalPivotAngle = WrapTo180Split(verticalPivotLocalRotation.eulerAngles.x);

            ApplyConstraints(ref horizontalPivotAngle, ref verticalPivotAngle);

            horizontalPivot.localRotation = Quaternion.Euler(0, horizontalPivotAngle, 0);
            verticalPivot.localRotation = Quaternion.Euler(verticalPivotAngle, 0, 0);
        }
        ...
        public virtual void Rotate(float horizontalRotation, float verticalRotation)
        {
            ...
            float horizontalAngle = horizontalPivot.localRotation.eulerAngles.y + horizontalRotation;
            float verticalAngle = verticalPivot.localRotation.eulerAngles.x + verticalRotation;

            ApplyConstraints(ref horizontalAngle, ref verticalAngle);

            horizontalPivot.localRotation = Quaternion.Euler(0f, horizontalAngle, 0f);
            verticalPivot.localRotation = Quaternion.Euler(verticalAngle, 0f, 0f);
        ...
        public virtual void ApplyConstraints(ref float horizontalAngle, ref float verticalAngle)
        {
            
            if (!noConstraintsHorizontal)
            {
                horizontalAngle = WrapTo180Split(horizontalAngle);
                horizontalAngle = Mathf.Clamp(horizontalAngle, minHorizontalPivotAngle, maxHorizontalPivotAngle);
            }

            verticalAngle = WrapTo180Split(verticalAngle);
            verticalAngle = Mathf.Clamp(verticalAngle, minVerticalPivotAngle, maxVerticalPivotAngle);
        }
        ...
        public virtual void ResetGimbalChild()
        {
            gimbalChild.localPosition = Vector3.zero;
            gimbalChild.localRotation = Quaternion.identity;
        }
