...
namespace VSX.Utilities
{
    ...
    public class LinkableVariable
    {
        ...
        {
            if (!isLinkedVariable) return true;
            if (targetComponent == null) return false;

            if (numArgs == 1)
            {
                if (Type.GetType(arg0Type) == null) return false;
                if (Type.GetType(arg0Type) == typeof(UnityEngine.Object) && arg0ObjectValue == null) return false;
                if (targetComponent.GetType().GetMethod(methodInfoName, new Type[] { Type.GetType(arg0Type) }) == null) return false;
            }
            else
            {
                if (targetComponent.GetType().GetMethod(methodInfoName) == null) return false;
            }
