...
namespace VSX.Health
{
    ...
    public class Damageable : MonoBehaviour
    {
        ...
        public virtual List<DamageReceiver> DamageReceivers { get => damageReceivers; set => damageReceivers = value; }
        ...
        public virtual void Register(DamageReceiver damageReceiver)
        {
            if (damageReceivers.IndexOf(damageReceiver) == -1) damageReceivers.Add(damageReceiver);
        }
        ...
        public virtual Vector3 GetClosestPoint(Vector3 position)
        {
            ...
            {
                Vector3 closestPoint = damageReceivers[i].GetClosestPoint(position);
                if (Vector3.Distance(closestPoint, position) < Vector3.Distance(result, position))
                {
                    result = closestPoint;
                }
            }
